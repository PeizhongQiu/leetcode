...
public:
    static bool cmp(replaceTip i,replaceTip j) { return (i.index > j.index); }
    string findReplaceString(string s, vector<int>& indices, vector<string>& sources, vector<string>& targets) {
        vector<replaceTip> replaceTips;
        for(int i = 0; i < indices.size(); i++) {
            replaceTip temp;
            temp.index = indices[i];
            temp.source = sources[i];
            temp.target = targets[i];
            replaceTips.push_back(temp);
        }
        sort(replaceTips.begin(), replaceTips.end(), Solution::cmp);
        for(int i = 0; i < indices.size(); i++) {
            string sub = s.substr(replaceTips[i].index, replaceTips[i].source.size());
            if(sub == replaceTips[i].source) {
                s.replace(replaceTips[i].index, replaceTips[i].source.size(), replaceTips[i].target);
            }
        }
        return s;
    }
