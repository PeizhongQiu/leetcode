 ...
 * [2065] 最大化一张图中的路径价值
 *
 * https://leetcode-cn.com/problems/maximum-path-quality-of-a-graph/description/
 *
 * algorithms
 * Hard (53.80%)
 * Likes:    18
...
public:
    ...
    void dfs(int tmp_value, int node, int curTime, int maxTime, vector<int>& values, vector<vector<EDGE>>& graph, vector<bool>& isVisit) {
        int change = 0;
        if(isVisit[node] == false) {
            tmp_value += values[node];
            isVisit[node] = true;
            change = 1;
        }
        if(node == 0) {
            ret = max(tmp_value, ret);
        }
        
        for(int i = 0; i < graph[node].size(); ++i) {
            if(curTime + graph[node][i].time <= maxTime) {
                dfs(tmp_value, graph[node][i].to, curTime + graph[node][i].time, maxTime, values, graph, isVisit);
            }
        }
    ...
    int maximalPathQuality(vector<int>& values, vector<vector<int>>& edges, int maxTime) {
        vector<vector<EDGE>> graph(values.size());
        vector<bool> isVisit(values.size(), false);
        for(int i = 0; i < edges.size(); ++i) {
            EDGE tmp = {edges[i][1], edges[i][2]};
            EDGE tmp2 = {edges[i][0], edges[i][2]};
            graph[edges[i][0]].push_back(tmp);
            graph[edges[i][1]].push_back(tmp2);
        }

        dfs(0, 0, 0, maxTime, values, graph, isVisit);
        return ret;
    }
};
// @lc code=end
